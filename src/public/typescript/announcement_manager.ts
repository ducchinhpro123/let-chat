...
class AnnouncementManager {
    ...
    removeAnnouncementItem(announcementId: string) {
        ...
        const announcementItem = announcementsElement?.querySelector(`.announcement-item[data-id="${announcementId}"]`);
    ...
    handleAccept(announcement: any, announcementId: string) {
        this.socketManager.emit('I accept to be a friend', announcement, (response: any) => {
            if (response.status === 'ok') {
    ...
    handleRefuse(announcement: any, announcementId: string) {
        this.socketManager.emit('I refuse to be a friend', announcement, (response: any) => {
            if (response.status === 'ok') {
                this.updateAnnouncementCount(response.announcementCount);
                this.removeAnnouncementItem(announcementId);
                this.closeModal();
