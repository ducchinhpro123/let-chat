...
import DateTimeFormatter from 'public/typescript/date_time_formatter';
...
class ConversationManager {
    ...
    constructor() {
        ...
        this.modalNameInput = document.getElementById('modalName') as HTMLInputElement;
    ...
    async handleDeleteConversation(conversationId: string) {
        try {
            ...
            this.socketManager.emit('delete this conversation', conversationId, (response: any) => {
    ...
    handleContextMenuAction(action: string, conversation: typeof Conversation) {
        switch (action) {
            case 'view':
            case 'mute':
            case 'mark-read':
    ...
    appendNewConversation(conversation) {
        ...
        const isYou = conversation.latestMessage.sender.username === currentUsername.innerHTML.trim() ? 
            'You' : conversation.latestMessage.sender.username
