...
class SocketManager {
    ...
    constructor() {
        this.socket = Socket;
        this.events = {};
        const userIdElement = document.getElementById('userid-hidden') as HTMLInputElement;
        if (!userIdElement) {
    ...
    getWSToken(): string | undefined {
        try {
            const cookies: {[key: string]: string} = document.cookie.split(';')
    ...
    setupEventListeners() {
        ...
        this.socket.on('One of you friend has added you to a conversation', (response: any) => {
